...
def load_data(args, dataset_name):  
    if dataset_name == "mnist":
        ...
        if args.partition_method == "centralized":
            ...
            class_num = load_partition_data_mnist(args.batch_size, partition_method="centralized")
            train_data_local_dict[0]=train_data_global
            test_data_local_dict[0]=test_data_global
        else:
            client_num = 10 #异构模式下，默认使用10个客户端
            client_num, train_data_num, test_data_num, train_data_global, test_data_global, \
            train_data_local_num_dict, train_data_local_dict, test_data_local_dict, \
            class_num = load_partition_data_mnist(args.batch_size,partition_method="hetero") #类别数(10)
        """
...
if __name__ == "__main__":
    ...
    if args.partition_method == "centralized":
        ...
        logging.info("Select Centralized Training, sample_num = %d" % (len(train_data)))
        ...
        raise Exception("Error: unrecognized partition method")
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title(f'{args.partition_method}:{args.dataset} Training on {args.model}:Training Time vs Loss')
