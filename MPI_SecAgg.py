        ...
        if rank == 0:  # Server
            ...
            for i in range(N):
                if drop_info[i] == 0:
                    ...
                    # print 'decoded s_sk info: (i,s_pk)',i,s_sk_dec

                    for j in range(N):
                        s_pk_list_ = public_key_list[1, :]
                        # print s_sk_dec[0][0], s_pk_list_[j]
                        # s_uv_dec = my_key_agreement(s_sk_dec[0][0], s_pk_list_[j], p, g)
                        s_uv_dec = np.mod(s_sk_dec[0][0] * s_pk_list_[j], p)
                        # print 's_uv_dec=',s_uv_dec

                        # print 'decoded s_uv_dec info: (i,j,s_pk)=',i,j,s_uv_dec
                        if j == i:
...
if rank == 0:
    pickle.dump(time_out, open('./SecAgg_N' + str(N) + '_d' + str(d), 'wb'), -1)
