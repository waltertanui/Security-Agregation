...
from sec_agg.mpc_function import LCC_decoding_with_points, LCC_encoding_with_points
from sec_agg.sparse_function import sparse_gradient,encode_gradient,decode_gradient
#初始化MPI环境
...
#第三部分——分布式计算与通信
    ...
    for avg_idx in range(n_trial): 
        ...
        if rank == 0:  # 服务器
            ...
            # 修改后代码
            # 假设稀疏化后每个用户只发送K个非零元素
            K = int(d * sparsity_ratio)
            x_tilde_buffer = np.zeros((U1, K), dtype=int)
            #------------------------------------------------------------------------------------------------
            ...
            #修改后代码
            decoded_grads = np.zeros((U1, d), dtype=int)
            for i in range(U1):
               decoded_grads[i] = decode_gradient(x_tilde_buffer[i], alpha_s, beta_s, p)
            x_tilde_sum = np.mod(np.sum(decoded_grads, axis=0), p)
            #--------------------------------------------------------------------------------------------------------------
...
#第5部分——数据处理与时间记录
            ...
            z_sum = np.reshape(z_tilde_buffer[0:U - T, :], (d,))
            ...
            for i in range(N):
                rx_rank = i + 1  #1~N
                tmp = np.empty(2, dtype='float')  #存储从客户端接收的时间数据
                comm.Recv(tmp, source=rx_rank)  #从rx_rank进程接收数据，存储在tmp中
        ...
        ##########################################
            ...
            #记录z_tlide缓冲区；大小为N*(d/(U-T))——存储所有用户编码数据z_tilde_i
            z_tilde_buffer = np.zeros(shape=(N, (d / (U - T)).astype(np.int64)), dtype=int)
            #将第rank个用户的编码数据 z_tilde_i 存入 z_tilde_buffer 的第rank-1行
            z_tilde_buffer[rank - 1, :] = z_tilde_i[rank - 1, :]

            #向其他用户发送z_LCC
            tx_req = [None] * (N - 1) #存储MPI的发送请求(长度为N-1)
            tx_dest = np.delete(range(N), rank - 1)  #生成(除自己外)需要发送数据的目标用户进程编号的列表
            ...
            # 修改后代码
            x_i = sparse_gradient(my_model, sparsity_ratio)  # 对模型梯度进行稀疏化处理
            #--------------------------------------------------------------------------------------
            time.sleep(5) #模拟训练时间
            ...
            if my_idx in surviving_set1:  #若当前索引为第一轮中存活索引之一，则计算并发送掩码模型给Server
                ...
                # 修改后代码
                encoded_grad = encode_gradient(x_i, alpha_s, beta_s, p)
                comm.Isend([encoded_grad, MPI.INT], dest=0)
                #-------------------------------------------------------------------------------------------------------
            #模拟通信延迟
            if is_sleep:
                data_size = d # 参数数量
            ...
            if my_idx in surviving_set2: #若当前索引为第二轮中存活索引之一，则计算并发送z_tilde_sum给Server
                ...
                comm.Isend([z_tilde_sum, MPI.INT], dest=0)
            #模拟通信延迟
            if is_sleep:
    ...
    if rank == 0:  #若为Server，记录日志，表示即将显示实验结果
        ...
        logging.info('time for offline = %s' % str(time_avg[2]))  #离线时间
...
#
#在线阶段第一轮和第二轮——User进程的数据生成、发送、通信延迟模拟；
#处理和保存结果——Server进程的结果的收集和存储